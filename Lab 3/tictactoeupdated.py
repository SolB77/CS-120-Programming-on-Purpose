...
#
# # Base Board
# boardfile = open("../Lab 1/Boards/tictactoe_board.txt")
...
def tic_tac_toe(board):
    ...
    game_board = board_file.read()
    board_file.close()
    rows = game_board.split('\n')
    board_size = len(rows)
    #print(board_size)
    # Print the list containing the split board sections unless the rows or columns are not full
    # FIXME Don't Think I Need This Row Check Section
    # if len(rows) != board_size:
    #     raise ValueError(f"The number of rows in the board is not equal to {board_size}!:\n"
    #                      f"{rows}. There are only {len(rows)} rows!")
    for row in rows:
        if len(row) != board_size:
            raise ValueError(f"The number of complete columns in the board is not equal to {board_size}!:\n"
                             f"{rows[row]}\n")
        else:
            print(f"The board looks like:\n"
                  f"{rows[row]}\n")
    for row in rows:
        # Print whether any of the rows are board_size in a row (either all player1_marker or player2_marker) and print the row position and values
        if all(cell == row[0] for cell in row):
            print(f"Row {rows.index(row)+1} has {len(rows)} {[row][0]}'s in a row:\n{row}\n")
        # Print whether any of the columns are board_size in a row (either all player1_marker or player2_marker) and print the column position and values
        # if rows[0][0] == rows[1][0] == rows[2][0]:
        #     print(f"Column 1 has three {rows[0][0]} in a row:\n"
        #           f"{rows[0][0]}\n"
        #           f"{rows[1][0]}\n"
        ...
        for x in range(len(rows)):
            if all()
            if rows[row][x] == rows[row][row]:
                print(f"Column  has three {rows[0][0]} in a row:\n"
                      f"{rows[0][0]}\n"
                      f"{rows[1][0]}\n"
                      f"{rows[2][0]}\n")
...
print(tic_tac_toe("../Lab 1/Boards/tictactoe_board.txt"))
